...
export function WorkflowDefinitionCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    createMutation.mutate(
      { name: values.name.trim(), documentTemplateId: selectedTemplateId, steps },
      {
        onSuccess: () => {
          notification.success({ message: t("workflow_definitions.notifications.created") });
          navigate("/app/steps");
        },
        onError: (error) => {
          const presentation = getApiErrorPresentation(error, t("workflow_definitions.notifications.create_failed_title"));
        ...
        <Space align="start" size={16}>
          ...
          <div>
            <Typography.Title level={3} style={{ margin: 0 }}>
              {t("workflow_definitions.create_page.title")}
            </Typography.Title>
            <Typography.Paragraph type="secondary" style={{ margin: "4px 0 0" }}>
              {t("workflow_definitions.create_page.description")}
